	...
	const { data: image } = useSuspenseQuery({
		...
		queryFn: async () => {
			...
			if ('bounds' in location) {
				search.set('bbox', location.bounds.join('~'));
			} else {
				const { center, zoom } = location as {
					center: LngLat;
					zoom: number;
				};
				search.set('ll', center.join(','));
				search.set('z', zoom.toString());
			}

			search.set('lang', language);
			search.set('size', '650,450'); // Max size
			search.set('apikey', process.env.APIKEY as string);
			image.src = `${process.env.staticApiUrl}?${search.toString()}`;
			await image.decode();
			return image;
		}
	});
